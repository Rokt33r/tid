...
const defaultMiddleware = [sessionMiddleware, withErrorHandler]

const p: typeof prismy = (selectors: any, handler: any, middleware: any = []) =>
  prismyx(selectors, handler, [...middleware, ...defaultMiddleware])

export default methodRouter({
  get: p([querySelector, sessionSelector], async (query, session) => {
    const { code } = query
